...
def schedule_test(email: str, uuid: str, random_words_password: str, song_password: str):
    ...
    __scheduler.add_job(send_test_mail, 'date', run_date=current_time + timedelta(seconds=1),
                        timezone=tzname[0], args=[email, uuid, 1])
    __scheduler.add_job(send_test_mail, 'date', run_date=current_time + timedelta(seconds=3),
                        timezone=tzname[0], args=[email, uuid, 3])
    __scheduler.add_job(send_test_mail, 'date', run_date=current_time + timedelta(seconds=7),
